...
func BuildFromConfig(cfg *config.Config) (*Registry, error) {
	...
	if inst := cfg.Arrs.WhisparrV2; inst.Enabled {
		c, err := whisparr_v2.New(whisparr_v2.Options{
			Name: string(triagearr.ArrTypeWhisparrV2), BaseURL: inst.URL, APIKey: inst.APIKey,
			Poll: inst.Poll, Act: inst.Act, Timeout: inst.Timeout,
		})
		if err != nil {
	...
	if inst := cfg.Arrs.WhisparrV3; inst.Enabled {
		c, err := whisparr_v3.New(whisparr_v3.Options{
			Name: string(triagearr.ArrTypeWhisparrV3), BaseURL: inst.URL, APIKey: inst.APIKey,
			Poll: inst.Poll, Act: inst.Act, Timeout: inst.Timeout,
		})
		if err != nil {
...
// connection failed. It does not touch the live registry. The Lidarr stub
// returns its "not implemented" health error.
func TestConnection(ctx context.Context, kind, baseURL, apiKey string, timeout time.Duration) error {
	...
	case triagearr.ArrTypeWhisparrV2:
		inst, err = whisparr_v2.New(whisparr_v2.Options{Name: "test", BaseURL: baseURL, APIKey: apiKey, Timeout: timeout})
	case triagearr.ArrTypeWhisparrV3:
		inst, err = whisparr_v3.New(whisparr_v3.Options{Name: "test", BaseURL: baseURL, APIKey: apiKey, Timeout: timeout})
	default:
...
// FileDeleter. The Lidarr stub deliberately does not — it fails (c) and is
// rejected here.
