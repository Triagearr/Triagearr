// Package fake is an in-memory Whisparr v2 (Sonarr-derived) /api/v3 API server
// suitable for integration tests and dev fixtures. Mirrors
// [github.com/Triagearr/Triagearr/internal/clients/arr/sonarr/fake]: Whisparr v2
// exposes series and episode files exactly like Sonarr.
//
// All state is in-memory; restarting the process resets it. Concurrent callers
// are serialized via a single RWMutex on State.
package fake
...
// SeriesStats is the nested statistics object Whisparr v2 returns. We only
// consume SizeOnDisk today; the rest is present for scenario authoring fidelity.
...
// HistoryRecord is one /api/v3/history entry. The convert function in
// [github.com/Triagearr/Triagearr/internal/clients/arr/arrhistory] is shared.
...
// HistoryRecordData carries the inner `data` blob Whisparr v2 returns inline.
...
// ListSeries returns a copy of all series (without the nested Files, which are
...
// FilesForSeries returns the episode files attached to a series, or nil if the
// series is unknown.
...
// EpisodeFileDeletes returns the number of successful DELETE /episodefile calls.
