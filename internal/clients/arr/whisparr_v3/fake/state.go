// Package fake is an in-memory Whisparr v3 (Radarr-derived) /api/v3 API server
// suitable for integration tests and dev fixtures. Mirrors
// [github.com/Triagearr/Triagearr/internal/clients/arr/radarr/fake]: Whisparr v3
// exposes movies and movie files exactly like Radarr.
package fake
...
// Movie mirrors the wire shape returned by /api/v3/movie. Whisparr v3, like
// Radarr, emits sizeOnDisk flat at the top level (no statistics nesting).
type Movie struct {
...
// HistoryRecord matches Radarr's shape — the convert function in
...
// HistoryRecordData carries the inner `data` blob Whisparr v3 returns inline.
...
// ListMovies returns all movies (without the nested Files, which are served by
// the per-movie endpoint).
...
// DeleteMovieFile removes a file from its parent movie. Returns true when the
// fileID existed.
