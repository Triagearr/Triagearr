...
// Sonarr maps numeric eventType filters to string event names. The real
// client uses eventType=3 → "downloadFolderImported"; we add the other
// common values for forward-compat scenarios.
...
func (s *Server) mux() *http.ServeMux {
	...
	m.HandleFunc("GET /api/v3/series", s.requireKey(s.handleSeries))
	m.HandleFunc("GET /api/v3/tag", s.requireKey(s.handleTags))
	m.HandleFunc("GET /api/v3/episodefile", s.requireKey(s.handleEpisodeFiles))
	m.HandleFunc("DELETE /api/v3/episodefile/{id}", s.requireKey(s.handleDeleteEpisodeFile))
...
func (s *Server) handleSeries(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, s.state.ListSeries())
}
...
func (s *Server) handleEpisodeFiles(w http.ResponseWriter, r *http.Request) {
	seriesIDRaw := r.URL.Query().Get("seriesId")
	if seriesIDRaw == "" {
		http.Error(w, "seriesId required", http.StatusBadRequest)
		return
	}
	seriesID, err := strconv.ParseInt(seriesIDRaw, 10, 64)
	if err != nil {
		http.Error(w, "invalid seriesId", http.StatusBadRequest)
		return
	}
	files := s.state.FilesForSeries(seriesID)
	if files == nil {
		// Sonarr returns [] for unknown seriesId rather than 404. Match that.
		writeJSON(w, []EpisodeFile{})
		return
	}
	writeJSON(w, files)
}
...
func (s *Server) handleDeleteEpisodeFile(w http.ResponseWriter, r *http.Request) {
	...
	ok := s.state.DeleteEpisodeFile(fileID)
	s.logger.Info("fake-sonarr: delete episodefile",
...
func (s *Server) handleUnknown(w http.ResponseWriter, r *http.Request) {
	s.logger.Warn("fake-sonarr: unimplemented endpoint",
