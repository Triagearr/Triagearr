...
`first_seen_dead` is recorded the first tick a tracker reports `status=4`, preserved as long as the tracker stays in `not_working`, and cleared on recovery (ADR-0013). qBit does not expose this timestamp directly — verified against the live `torrents/trackers` API (fields: `url, status, tier, num_peers, num_seeds, num_leeches, num_downloaded, msg`, plus undocumented `min_announce`/`next_announce` which are *future* reannounce schedules) and the official WebUI-API docs. `last_checked` is rewritten every poll (default 6h) and therefore cannot be used to measure "sustained dead".

**Seeding from an activity proxy (not `now`).** Because qBit carries no "tracker died at" timestamp, the first-observation tick would naively stamp `first_seen_dead = now` — making the grace clock measure *Triagearr's observation history* rather than the tracker's real death. The two diverge whenever Triagearr meets an already-dead tracker fresh: a torrent re-added or cross-seeded from content that has been dead for years would still have to "serve" a 7-day grace it factually outlived. (A wiped DB is the same situation applied to the whole library at once — relevant during alpha, where the schema is reset rather than migrated.) Instead, the alive→dead transition seeds `first_seen_dead` from `min(now, activity_proxy)`, where `activity_proxy = COALESCE(torrents.last_activity, completion_on, added_on)` (a single race-free read of the torrents row the poller already maintains). A torrent whose swarm went quiet months ago therefore qualifies on the first poll, while a torrent that was active moments ago (recent `last_activity`) still serves out the full grace when its tracker merely blips — preserving the anti-transient-outage guarantee. Seeding happens only on the transition; sustained-dead preservation and dead→alive→dead reset are unchanged.
